...
void Test_ATCFlight::test_setProfileSpeed()
{
    ...
    ATCProfileSpeed *p = new ATCProfileSpeed(levels, clbSpds, crsSpds, dscSpds);

    ATCFlight foo;
    QVERIFY(foo.getProfileSpeed() == nullptr);

    foo.setProfileSpeed(p);
    QVERIFY(foo.getProfileSpeed() == p);
}
